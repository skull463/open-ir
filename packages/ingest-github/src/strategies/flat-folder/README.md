...
- `analyse-changed.ts` — `analyseChangedFiles({knowledgeId, source, metaPaths, analyzer, diff, llmCallContext?, archiveSink?, progressContext?, ignoreSets?})`. Pull-time per-file dispatcher. Reads changed file content through `input.source` (a `SourceReader`) so it works with both the disk-backed reader (OSS default) and any HTTP-backed alternative supplied via the `pullFactory` hook. Mirrors `analyseSmallFiles`'s per-file path: static-ignore check → fetch → size cap → binary detect → line count → analyse → save + archive push. Does NOT invoke the skip-decision LLM gate. `ignoreSets` (per-job effective ignore sets, see `pipeline/skip-decisions/effective.ts`) makes the static check match index-time filtering — directory segments and globs included, since walk-time pruning never runs on diff paths. When `progressContext` is present it creates a fixed-total reporter (`subPhase: "pull"`, `total = dedupedPaths.length`) and increments per-path so the pull SSE stream stays live.
- `analyse-changed-helpers.ts` — `isStaticallyIgnored` (index-equivalent static skip decision for one diff path), `mergeBigFileEntries`, `describe`. Split out of `analyse-changed.ts` to keep it under the 300-line ceiling.
